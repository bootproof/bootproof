...
// ---------------------------------------------------------------------------
// Convert a TS CLI Attestation into the receipt's internal record format.
// The browser-side JS works with a simpler shape; we map the rich Attestation
// schema into it without losing any evidence.
// ---------------------------------------------------------------------------
interface ReceiptRecord {
  ...
  plan: Array<{ step: string; command: string; exitCode: number; durationMs: number }>;
  log: Array<{ t: number; level: string; line: string }>;
  observed: {
    kind: string;
    url?: string;
    status?: number;
...
function attestationToRecord(att: Attestation, inference?: Inference | null): ReceiptRecord {
  ...
  if (inference) {
    ...
    if (startStep) log.push({ t: 0, level: "info", line: `  inferred start: ${startStep.command || startStep.description}` });
    if (att.plan.healthUrl) log.push({ t: 0, level: "info", line: `  health candidate: ${att.plan.healthUrl}` });
  }

  // Plan steps from observed
  const plan: ReceiptRecord["plan"] = [];
  for (const step of att.observed as ObservedStep[]) {
    ...
    if (step.command) log.push({ t: stepStart, level: "info", line: `  running: ${step.command}` });

    // Add evidence lines
    if (step.evidenceHead) {
      for (const line of step.evidenceHead.split("\n").slice(0, 5)) {
        if (line.trim()) log.push({ t: stepStart, level: "info", line: `  > ${line}` });
      }
    }
    ...
    if (step.evidenceTail) {
      for (const line of step.evidenceTail.split("\n").slice(-5)) {
        if (line.trim()) log.push({ t: stepEnd, level: "info", line: `  > ${line}` });
      }
    }
  ...
  // Observed evidence
  let observed: ReceiptRecord["observed"];
...
// ---------------------------------------------------------------------------
function buildHtml(record: ReceiptRecord, signature: string, publicKeySpkiB64: string): string {
...
<div class="wrap">
  <section class="hero">
    ...
    <p class="lede">A single self-contained file. When you opened it, your browser ran <b>Ed25519 verification</b> locally &mdash; no network, no install, no account. If a single byte of any signed message is altered, the receipt's verdict collapses with it.</p>
    <span class="real-badge">Real boot captured by bootproof up &middot; not mock data</span>
    <div class="first-time" id="firstTime"><div class="ft-body"><div class="ft-title">You're holding a Living Receipt</div><div class="ft-text">Someone ran <b>bootproof up</b> on a real repository, captured real evidence, and signed it. This file re-proves that to you, offline. Forward it &mdash; it verifies itself on the next machine.</div><a class="ft-cta" href="https://github.com/bootproof/bootproof#readme" target="_blank" rel="noopener">Get your own receipt &rarr;</a></div><button class="ft-dismiss" id="ftDismiss" aria-label="dismiss">&times;</button></div>
    <div class="status-bar" id="statusBar"><span class="dot"></span><span id="statusText">Verifying signatures in your browser&hellip;</span></div>
  </section>
  <section class="receipts" id="receipts"></section>
  <div class="cta-bar"><h2>Got a repo that needs proof?</h2><p>Point BootProof at any repository. It boots what it can, refuses what it can't prove, and signs the receipt.</p><div class="cmd-display"><span class="dollar">$</span> npx bootproof up &lt;any-repo-url&gt; --receipt</div><br><a class="cta-link" href="https://github.com/bootproof/bootproof#readme" target="_blank" rel="noopener">github.com/bootproof/bootproof &rarr;</a></div>
  <footer class="page">
    ...
    <p style="margin-top:14px"><b>Trust ladder.</b> This receipt is signed at the <code>${record.trust.level}</code> level. The documented upgrade path is: <code>local_developer_signed</code> &rarr; <code>ci_oidc_signed</code> &rarr; <code>neutral_runner_signed</code> &rarr; <code>transparency_logged</code>. The same file format survives every step; only the signer's identity climbs.</p>
    ...
    <p style="margin-top:14px"><a href="https://boot-proof.com">boot-proof.com</a> &nbsp;&middot;&nbsp; <a href="https://github.com/bootproof/bootproof">github.com/bootproof/bootproof</a> &nbsp;&middot;&nbsp; <code>npx bootproof up &lt;any repo url&gt; --receipt</code></p>
  </footer>
</div>
